...
inline void GetAssetsPath(_Out_writes_(pathSize) wchar* path, u32 pathSize)
{
	if (path == nullptr)
	{
		throw std::exception();
...
public:
	...
	virtual EExitCode initialize() override
	{
		...
		_swapchain.create(1, 800, 600, true);
...

		{
			...
			GetAssetsPath(workspacePath, std::size(workspacePath));
			std::wstring shaderPath(workspacePath);
			std::wstring vspath{shaderPath + L"shaders.hlsl"};
			std::wstring pspath{shaderPath + L"shaders.hlsl"};
	...
	void PopulateCommandList()
	{
		...
		_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(_renderTargets[_frameIndex].getReference(), D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET));
		...
		_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(_renderTargets[_frameIndex].getReference(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT));
